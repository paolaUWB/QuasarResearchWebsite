...
SELECT 
     REVERSE(PARSENAME(REPLACE(REVERSE(PLATE_MJD_FIBER), '-', '.'), 1)) AS [Plate]
   , REVERSE(PARSENAME(REPLACE(REVERSE(PLATE_MJD_FIBER), '-', '.'), 2)) AS [MJD]
   , REVERSE(PARSENAME(REPLACE(REVERSE(PLATE_MJD_FIBER), '-', '.'), 3)) AS [Fiber]
FROM quasarinfo;
GO 
